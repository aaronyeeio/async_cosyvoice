...
class AsyncCosyVoice2:
    def __init__(self, model_dir, load_jit=False, load_trt=False, fp16=False):
        ...
        self.model = CosyVoice2Model(model_dir, configs["flow"], configs["hift"], fp16)
        self.model.load(
    ...
    async def inference_sft(
        ...
        for i in tqdm(
            ...
            async for model_output in self.model.async_tts(
    ...
    async def inference_instruct2_by_spk_id(
        self,
        tts_text,
        instruct_text,
        spk_id,
        ...
        for i in tqdm(
            ...
            model_input = self.frontend.frontend_instruct2_by_spk_id(
                i, instruct_text, spk_id
            )
            start_time = time.time()
            logging.info("synthesis text {}".format(i))
            async for model_output in self.model.async_tts(
    ...
    async def inference_zero_shot_by_spk_id(
        self, tts_text, spk_id, stream=False, speed=1.0, text_frontend=True
    ):
                ...
                **model_input, stream=stream, speed=speed
                ...
                logging.info(
                    "yield speech index:{}, len {:.2f}, rtf {:.3f},  cost {:.3f}s,  all cost time {:.3f}s".format(
                        chunk_index,
                        speech_len,
                        (time.time() - last_time) / speech_len,
                        time.time() - last_time,
